...
export const useBannerColumns = () => {
  ...
  const getBannerStatus = (banner: BannerInclude) => {
    ...
    if (!banner.isActive) {
      return {
        label: t("banners.status.inactive"),
        variant: "secondary" as const,
      }
    }
    if (now < startDate) {
      return {
        label: t("banners.status.scheduled"),
    ...
    return {
      label: t("banners.status.expired"),
      ...
      cell: ({ row }) => {
        ...
        return (
          <HoverCard>
            <HoverCardTrigger
              render={
                <Badge className="cursor-default text-sm font-medium">
                  {users.length}
                </Badge>
              }
            />
            <HoverCardContent>
              <div className="flex flex-wrap gap-1">
                {users.map((user) => (
                  <UserAvatar size="sm" key={user.id} {...user} />
                ))}
              </div>
            </HoverCardContent>
          </HoverCard>
